...
global PDFReader MP4Reader GoogleEarthReader GimpReader SnapReader SnapGpt S1tbxReader MapReadyReader ImageMagickMaker
...
if {$OpenDirFile == 0} {
    ...
    if {$ConfigFileNameSearch == "MP4"} {
        set MP4Reader $ConfigFileNamePath
        if {$PlatForm == "windows"} {
            if {$MP4Reader != ""} {
                set f [open "$CONFIGDir/MP4ReaderWindows.txt" w]
                puts $f $MP4Reader
                close $f
                }
            }
        if {$PlatForm == "unix"} {
            if {$MP4Reader != ""} {
                set f [open "$CONFIGDir/MP4ReaderUnix.txt" w]
                puts $f $MP4Reader
                close $f
                }
            }       
    ...
    if {$ConfigFileNameSearch == "MP4"} { TextEditorRunTrace "Close Window Configuration MP4 VIEWER Software" "b" }
