...
Description :  Convert SAOCOM Binary Data Files 
               (GEOTIFF Format)
...
strcat(UsageHelp," (int)   	-off  	Offset Bytes\n");
strcat(UsageHelp," (int)   	-pre  	Prefix Bytes\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
if(argc < 37) {
  ...
  get_commandline_prm(argc,argv,"-off",int_cmd_prm,&PointerPosition,1,UsageHelp);
  get_commandline_prm(argc,argv,"-pre",int_cmd_prm,&Prefix,1,UsageHelp);
...
for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], PointerPosition, SEEK_CUR);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) {
      fseek(in_file[Np], Prefix, SEEK_CUR);
      if (IEEE == 0)
