...
Description :  Convert RADARSAT-RCM  Binary Data Files 
               (GEOTIFF Format)
...
/* LOCAL VARIABLES */
  ...
  char LutFile11[FilePathLength],LutFile12[FilePathLength],LutFile21[FilePathLength],LutFile22[FilePathLength];
...
strcat(UsageHelp," (string)	-lt1 	input lut file: s11\n");
strcat(UsageHelp," (string)	-lt2 	input lut file: s12\n");
strcat(UsageHelp," (string)	-lt3 	input lut file: s21\n");
strcat(UsageHelp," (string)	-lt4 	input lut file: s22\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
if(argc < 45) {
  ...
  get_commandline_prm(argc,argv,"-lt1",str_cmd_prm,LutFile11,1,UsageHelp);
  get_commandline_prm(argc,argv,"-lt2",str_cmd_prm,LutFile12,1,UsageHelp);
  get_commandline_prm(argc,argv,"-lt3",str_cmd_prm,LutFile21,1,UsageHelp);
  get_commandline_prm(argc,argv,"-lt4",str_cmd_prm,LutFile22,1,UsageHelp);
...
********************************************************************/
  ...
  check_file(File22);
  check_dir(out_dir);
  check_file(LutFile11);
  check_file(LutFile12);
  check_file(LutFile21);
  check_file(LutFile22);
...
/* DATA FILES */
  ...
  LutArray = matrix_float(Npol,Ncol);
  if ((in_lut = fopen(LutFile11, "rb")) == NULL)
    edit_error("Could not open input file : ", LutFile11);
  fread(&LutArray[0][0], sizeof(float), Ncol, in_lut);
  fclose(in_lut);
  if ((in_lut = fopen(LutFile12, "rb")) == NULL)
    edit_error("Could not open input file : ", LutFile12);
  fread(&LutArray[1][0], sizeof(float), Ncol, in_lut);
  fclose(in_lut);
  if ((in_lut = fopen(LutFile21, "rb")) == NULL)
    edit_error("Could not open input file : ", LutFile21);
  fread(&LutArray[2][0], sizeof(float), Ncol, in_lut);
  fclose(in_lut);
  if ((in_lut = fopen(LutFile22, "rb")) == NULL)
    edit_error("Could not open input file : ", LutFile22);
  fread(&LutArray[3][0], sizeof(float), Ncol, in_lut);
  fclose(in_lut);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
        S_in[Np][lig][2*col] = S_in[Np][lig][2*col] / LutArray[Np][col];
        S_in[Np][lig][2*col+1] = S_in[Np][lig][2*col+1] / LutArray[Np][col];
