...
Description :  extract information from a CSG header file
...
/* LOCAL VARIABLES */
  ...
  char Frequency[FilePathLength], NearIncAngle[FilePathLength], FarIncAngle[FilePathLength], Satellite[FilePathLength], SceneStart[FilePathLength], SceneStop[FilePathLength];
  char Polarisation[FilePathLength], ProductType[FilePathLength], Column[FilePathLength], Line[FilePathLength];
...
*/
    ...
    if (strstr(Buf,"Polarization") != NULL) {
      for (ii=0; ii<9; ii++) fgets(&Buf[0], 1024, ftmp);
      FlagChar = 0; ii = -1;
      while (FlagChar == 0) {
        ii++;
      ...
      ii++;
      strcpy(Polarisation, ""); strncat(Polarisation, &Buf[ii], strlen(Buf) - ii - 2);
      for (ii=0; ii<2; ii++) fgets(&Buf[0], 1024, ftmp);
      }

    if (strstr(Buf,"Product Type") != NULL) {
      for (ii=0; ii<9; ii++) fgets(&Buf[0], 1024, ftmp);
      FlagChar = 0; ii = -1;
      while (FlagChar == 0) {
        ii++;
      ...
      ii++;
      strcpy(ProductType, ""); strncat(ProductType, &Buf[ii], strlen(Buf) - ii - 2);
      for (ii=0; ii<2; ii++) fgets(&Buf[0], 1024, ftmp);
      }
    ...
    if (strstr(Buf,"Far Incidence Angle") != NULL) {
      for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
      FlagChar = 0; ii = -1;
      while (FlagChar == 0) {
        ii++;
      ...
      ii++;
      strcpy(FarIncAngle, ""); strncat(FarIncAngle, &Buf[ii], strlen(Buf) - ii - 1);
      for (ii=0; ii<2; ii++) fgets(&Buf[0], 1024, ftmp);
      }

    if (strstr(Buf,"Near Incidence Angle") != NULL) {
      for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
      FlagChar = 0; ii = -1;
      while (FlagChar == 0) {
        ii++;
      ...
      ii++;
      strcpy(NearIncAngle, ""); strncat(NearIncAngle, &Buf[ii], strlen(Buf) - ii - 1);
      for (ii=0; ii<2; ii++) fgets(&Buf[0], 1024, ftmp);
      }
    ...
    if (strstr(Buf,"Scene Centre Geodetic Coordinates") != NULL) {
      for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
      FlagChar = 0; ii = -1;
      while (FlagChar == 0) {
        ii++;
    ...
    if (strstr(Buf,"GROUP \"S01\"") != NULL) {
      ...
      while (FlagGroup == 0) {
        ...
        if (strstr(Buf,"DATASET \"IMG\"") != NULL) {
          fgets(&Buf[0], 1024, ftmp); 
          fgets(&Buf[0], 1024, ftmp); 
          FlagChar = 0; ii = -1;
          while (FlagChar == 0) {
          ...
          strcpy(Tmp, ""); strncat(Tmp, &Buf[ii], strlen(Buf) - ii - 1);
          sscanf(Tmp, "DATASPACE  SIMPLE { ( %i, %i, %i ) / ( %s, %s, %i ) }", &Nlig, &Ncol, &ii, Tmp, Tmp, &ii);
          }

        if (strstr(Buf,"ATTRIBUTE \"Bottom Left Geodetic Coordinates\"") != NULL) {
          for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
          FlagChar = 0; ii = -1;
          while (FlagChar == 0) {
            ii++;
        ...
        if (strstr(Buf,"ATTRIBUTE \"Bottom Right Geodetic Coordinates\"") != NULL) {
          for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
          FlagChar = 0; ii = -1;
          while (FlagChar == 0) {
            ii++;
        ...
        if (strstr(Buf,"ATTRIBUTE \"Top Left Geodetic Coordinates\"") != NULL) {
          for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
          FlagChar = 0; ii = -1;
          while (FlagChar == 0) {
            ii++;
        ...
        if (strstr(Buf,"ATTRIBUTE \"Top Right Geodetic Coordinates\"") != NULL) {
          for (ii=0; ii<4; ii++) fgets(&Buf[0], 1024, ftmp);
          FlagChar = 0; ii = -1;
          while (FlagChar == 0) {
            ii++;
