...
/* Set the output matrix to 0 */
  ...
  for (sub = sub_init; sub < sub_number; sub++) {
    ...
    Nvalid = 0.; Mmean = 0.; Mmean2 = 0.; value = 0.; mean = 0.; mean2 = 0.;
#pragma omp parallel for private(col, k, l, Nvalid, Mmean, Mmean2, mean, mean2, value)
    for (lig = 0; lig < NligBlock[Nb]; lig++) {
  ...
  CV = 0.;  
#pragma omp parallel for private(col, CV)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    if (omp_get_thread_num() == 0) PrintfLine(lig,NligBlock[Nb]);
    for (col = 0; col < Sub_Ncol; col++) {
