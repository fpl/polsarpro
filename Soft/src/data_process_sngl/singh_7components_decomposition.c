...
File     : singh_7components_decomposition.c
...
Description :  Singh 7 components Decomposition
...
/*Rotation is not required for 7SD, teta = 0*/
/********************************************************************
*********************************************************************
*
*            -- Function : Main
...
#define NPolType 3
/* LOCAL VARIABLES */
  FILE *out_odd, *out_dbl, *out_vol, *out_hlx, *out_od, *out_cd, *out_md;
...
/* Internal variables */
  ...
  float Ps, Pd, Pv, Ph, Pod, Pcd, Pmd, Pmw, Pcw, TP;
...
strcpy(UsageHelp,"\nsingh_7components_decomposition.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sSingh_7SD_Odd.bin", out_dir);
  if ((out_odd = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_Dbl.bin", out_dir);
  if ((out_dbl = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_Vol.bin", out_dir);
  if ((out_vol = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_Hlx.bin", out_dir);
  if ((out_hlx = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_OD.bin", out_dir);
  if ((out_od = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_CD.bin", out_dir);
  if ((out_cd = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

 sprintf(file_name, "%sSingh_7SD_MD.bin", out_dir);
  if ((out_md = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_red.bin", out_dir);
  if ((out_red = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_green.bin", out_dir);
  if ((out_green = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh_7SD_blue.bin", out_dir);
...
/*-----------------------------------------------------------------*/   
  ...
  M_md = matrix_float(NligBlock[0], Sub_Ncol);
...
Ps = Pd = Pv = Ph = Pod = Pcd = Pmd = Pcw = Pmw = TP = 0.;
#pragma omp parallel for private(col, Np, TT, M_avg) firstprivate(ratio, S, D, Cre, Cim, C0, C1, teta, Ps, Pd, Pv, Ph, Pod, Pcd, Pcw,Pmd,Pmw, TP) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    TT = vector_float(NpolarOut);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        //Orientation Angle Compensation is not required for 7SD *****/
        teta = 0.;
        unitary_rotation(TT,teta);

        Pod = 2. * fabs(TT[T313_re]);
        Pcd = 2. * fabs(TT[T313_im]);
        Pmd = 2. * fabs(TT[T323_re]);
        Pv = 2.*TT[T333] - Ph - Pod - Pcd - Pmd;
        Pcw = Pod + Pcd;
        Pmw = Pmd + Ph;

        C1 = TT[T311] - TT[T322] + (7.*TT[T333]/8.) + (Ph/16.) + (Pmd/16.) - (15.*Pod/16.) - (15.*Pcd/16.);

        if (C1 > 0.) {
          if (Pv < 0.) {
            Ph = 0.; Pod = 0.; Pcd = 0.; Pmd = 0.; 
            Pv = 2.*TT[T333];
            }
          ...
          if (ratio <= -2.) {
            ...
            D = TT[T322] - (7.*Pv/30.) - (Pmw/2.);
            Cre = TT[T312_re] - (Pv/6.); Cim = TT[T312_im];
            }
          if ((ratio > -2.)&&(ratio <= 2.)) {
            Pv = 2.*Pv; 
            S = TT[T311] - (Pv/2.) - (Pcw/2.);
            D = TT[T322] - (Pv/4.) - (Pmw/2.);
          ...
          if (Pv + Pmw + Pcw > TP) {
            Ps = 0.; Pd = 0.; Pv = TP - Pmw - Pcw;
            } else {
            C0 = 2.*TT[T311] + Ph + Pmd - TP;
            if (C0 > 0) {
            ...
            if ((Ps > 0.)&&(Pd <0.)) {
              Pd = 0.; Ps = TP - Pv - Pmw - Pcw;
              }
            if ((Pd > 0.)&&(Ps <0.)) {
              Ps = 0.; Pd = TP - Pv - Pmw - Pcw;
              }
            }
          } else {
          if (Pv < 0.) {
        ...
        if (Ps < 0.) Ps = 0.; if (Pd < 0.) Pd = 0.;
        if (Pv < 0.) Pv = 0.; if (Ph < 0.) Ph = 0.;
        if (Pod < 0.) Pod = 0.; if (Pcd < 0.) Pcd = 0.;
        if (Pmd < 0.) Pmd = 0.;

        if (Ps > SpanMax) Ps = SpanMax; if (Pd > SpanMax) Pd = SpanMax;
        if (Pv > SpanMax) Pv = SpanMax; if (Ph > SpanMax) Ph = SpanMax;
        if (Pod > SpanMax) Pod = SpanMax; if (Pcd > SpanMax) Pcd = SpanMax; if (Pmd > SpanMax) Pmd = SpanMax;
        ...
        // A simple RGB color code based on Pd, PV and Ps is sufficient. The following color code can be ignored ********/
        M_blue[lig][col] = Ps + (2./5.)*(Pcd + Pod) ;
        M_green[lig][col] = Pv + (2./5.)*(Pcd + Pod) + (2./5.)*(Ph + Pmd) ;
        M_red[lig][col] = Pd + (1./5.)*(Pcd + Pod) + (3./5.)*(Ph + Pmd);
/***********************************************************/
        } else {
        M_odd[lig][col] = 0.; M_dbl[lig][col] = 0.;
        M_vol[lig][col] = 0.; M_hlx[lig][col] = 0.;
        M_od[lig][col] = 0.; M_cd[lig][col] = 0.;
        M_md[lig][col] = 0.; M_red[lig][col] = 0.; 
        M_green[lig][col] = 0.; M_blue[lig][col] = 0.;
        }
      }
    free_matrix_float(M_avg,NpolarOut);
  ...
  write_block_matrix_float(out_md, M_md, NligBlock[Nb], Sub_Ncol, 0, 0, Sub_Ncol);
